...
function EditAvatarPopup() {
	const isOpen = useAppSelector(
		(state) => state.popupsVisibility.editAvatarPopupIsVisible,
	);
	const dispatch = useAppDispatch();

	const { values, handleChange, setValues } = useForm();

	const [updateAvatar, { isLoading: isUpdating, isSuccess, reset }] =
	...
	return (
		<PopupWithForm
			...
			onSubmit={(event) => {
				...
				reset();
				updateAvatar({ avatar: values[InputName.avatarLink] });
			}}
		>
